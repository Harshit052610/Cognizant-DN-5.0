# Git HOL 1: Setup and Configuration
...
**Note:** Please follow the below steps for creating a free account in GitHub. Don't use cognizant credentials to login to GitHub.
...
- To check if Git client is installed properly: Open Git bash shell and execute 

If output shows Git with its version information that indicates, that Git Client installs properly.

- To configure user level configuration of user ID and email ID execute 

- To check if the configuration is properly set, execute the following command.
...
- To check if local and "Working Directory" git repository are same, execute git status
...
- To push the local to remote repository, execute 
...
- Git client is installed and configured with user ID and email
- Notepad++ is integrated as the default editor
- "welcome.txt" file is created and committed
- File is successfully pushed to remote repository
